...
def test_dataset():
    ...
    X_val = pd.read_csv(os.path.join(os.path.dirname(__file__), 'train\X_val.csv'))
    y_val = pd.read_csv(os.path.join(os.path.dirname(__file__), 'train\y_val.csv'))
    ...
    print(calculate_metric(model, X_val, y_val, mse))
    assert calculate_metric(model, X_val, y_val, mse) < 3.0, 'MSE превышает 3.0'


def test_dataset1():
    X_train = pd.read_csv(os.path.join(os.path.dirname(__file__), 'test\X_train.csv'))
    y_train = pd.read_csv(os.path.join(os.path.dirname(__file__), 'test\y_train.csv'))
    ...
    assert calculate_metric(model, X_train, y_train, mse) < 3.0, 'MSE превышает 3.0'
